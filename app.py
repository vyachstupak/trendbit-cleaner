...
from reddit_cleaner import clean_reddit_items
...
# ---------- Reddit endpoints ----------
@app.post("/clean/reddit")
def clean_reddit(req: CleanOneRequest):
    try:
        rows = clean_reddit_items([req.item], req.category)
        return {"row": rows[0] if rows else None}
    except Exception as e:
        tb = traceback.format_exc()
        raise HTTPException(status_code=500, detail={"error": str(e), "traceback": tb[-4000:]})

@app.post("/clean/reddit/batch")
def clean_reddit_batch(req: CleanBatchRequest):
    try:
        rows = clean_reddit_items(req.items, req.category)
        return {"rows": rows}
    except Exception as e:
        tb = traceback.format_exc()
        raise HTTPException(status_code=500, detail={"error": str(e), "traceback": tb[-4000:]})