...
def clean_instagram_df(df: pd.DataFrame, category: str, now_utc: Optional[datetime] = None) -> pd.DataFrame:
    if now_utc is None:
...
def clean_instagram_items(items: List[Dict[str, Any]], category: str):
    df = pd.DataFrame(items)
    if df.empty:
        return []
    clean_df = clean_instagram_df(df, category)
